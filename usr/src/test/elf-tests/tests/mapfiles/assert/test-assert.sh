...
function should_succeed {
    ...
    if gcc -shared $LDFLAGS -Wl,-M,${TESTDIR}/$mapfile \
...
function should_fail {
    ...
    if gcc -shared $LDFLAGS -Wl,-M,${TESTDIR}/$mapfile \
           ...
           /usr/xpg4/bin/grep -q "$error"; then
        echo "pass: $msg"
    else
        echo "FAIL: $msg"
        ret=1
...
should_fail mapfile.wrongtype "link with incorrect type in object (data v. function)" \
    "ld: fatal: .*mapfile.wrongtype: 35: assertion failed: type of symbol data should be:"

should_fail mapfile.wrongtype2 "link with incorrect type in object (common v. data)" \
    "ld: fatal: .*mapfile.wrongtype2: 35: assertion failed: type of symbol data should be:"

should_fail mapfile.wrongsize "link with incorrect size in object" \
    "ld: fatal: .*mapfile.wrongsize: 35: assertion failed: size of symbol data should be:"

should_fail mapfile.wrongscope "link with incorrect scope in object" \
    "ld: fatal: .*mapfile.wrongscope: 35: assertion failed: scope of symbol function should be:"

should_fail mapfile.wrongbits "link with incorrect shattr in object (nobits when bits)" \
    "ld: fatal: .*mapfile.wrongbits: 35: assertion failed: symbol [^ ]* is not in an SHT_NOBITS section"

should_fail mapfile.wrongbits2 "link with incorrect shattr in object (bits when nobits)" \
    "ld: fatal: .*mapfile.wrongbits2: 35: assertion failed: symbol [^ ]* is in an SHT_NOBITS section"
...
should_fail mapfile.wrongalias "link with incorrect alias" \
    "ld: fatal: .*mapfile.wrongalias: 35: assertion failed: symbol weak_function is not an alias of common"
...
should_fail mapfile.unknown-alias "link with alias to unknown symbol" \
    "ld: fatal: .*mapfile.unknown-alias: 35: assertion failed: unknown symbol in ALIAS: undefined_symbol"
...
should_fail mapfile.not-alias "link with two non-alias of the same value" \
     "ld: fatal: .*mapfile.not-alias: 64: weak_data and data (.*mapfile.not-alias: 43)"

should_fail mapfile.no-extern "link with assertions on an extern symbol" \
     "ld: fatal: .*mapfile.no-extern: 85: can't assert attributes of extern/parent symbol: extern"

should_fail mapfile.no-parent "link with assertions on a parent symbol" \
            "ld: fatal: .*mapfile.no-parent: 85: can't assert attributes of extern/parent symbol: parent"

should_fail mapfile.circalias "link with alias of alias" \
     "ld: fatal: .*mapfile.circalias: 35: weak_function should not be aliased to an alias"
