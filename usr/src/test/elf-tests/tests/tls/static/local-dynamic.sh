...
gcc $cflags64 -Wall -Wextra -Werror ${TESTDIR}/bin.c -DMODEL='"local-dynamic"' \
    -o local-dynamic.64
if (( $? != 0 )); then
	print -u2 "Couldn't compile ${TESTDIR}/bin.c (local-dynamic.64)"
	exit 1;
fi
...
gcc -m32 -Wall -Wextra -Werror ${TESTDIR}/bin.c -DMODEL='"local-dynamic"' \
    -o local-dynamic.32
if (( $? != 0 )); then
	print -u2 "Couldn't compile ${TESTDIR}/bin.c (local-dynamic.32)"
	exit 1;
fi
