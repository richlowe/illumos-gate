...
gcc -m32 -Wall -Wextra -Werror -fPIC -shared ${TESTDIR}/lib.c -o libinitial-exec32.so
if (( $? != 0 )); then
	print -u2 "Couldn't compile ${TESTDIR}/lib.c (libinitial-exec32.so)"
	exit 1;
fi

gcc -m32 -Wall -Wextra -Werror ${TESTDIR}/bin.c -DMODEL='"initial-exec"' \
    -o initial-exec.32 -L. -R'$ORIGIN' -linitial-exec32
if (( $? != 0 )); then
