...
./local-exec.64
if (( $? != 0 )); then
	print -u2 "Execution test failed (local-exec.64)"
	exit 1;
fi
...
gcc -m32 -Wall -Wextra -Werror ${TESTDIR}/bin.c -DMODEL='"local-exec"' \
    -o local-exec.32
if (( $? != 0 )); then
	print -u2 "Couldn't compile ${TESTDIR}/bin.c (local-exec.32)"
	exit 1;
fi

./local-exec.32
if (( $? != 0 )); then
	print -u2 "Execution test failed (local-exec.32)"
	exit 1;
fi
