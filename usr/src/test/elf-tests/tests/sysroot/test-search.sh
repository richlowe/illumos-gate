...
if [[ $PWD != $tmpdir ]]; then
	misc_fail "Failed to create temporary directory: $tmpdir"
fi
...
ld -Dlibs,detail -64 -lc | grep -q 'find lib=-lc; path=/lib/64/libc.so'
if (( $? != 0 )); then
	ret=1
...
ld -Dlibs,detail -64 -z sysroot=$PWD/default -lc | \
...
ld -Dlibs,detail -64 -z sysroot=$PWD/non-default -L'$SYSROOT/' -lc | \
...
ld -Dlibs,detail -64 -z sysroot=$PWD/non-default -L'=/' -lc | \
    grep -q "find lib=-lc; path=$PWD/non-default//libc.so"
if (( $? != 0 )); then
	ret=1
...
ld -Dlibs,detail -64 -z assert-deflib -z fatal-warnings \
    -z sysroot=$PWD/default -lc 2>&1 | \
    grep -q "dynamic library found on default search path"
if (( $? == 0 )); then
	ret=1
	print -u2 "FAIL: -zassert-deflib doesn't respect SYSROOT"
fi
