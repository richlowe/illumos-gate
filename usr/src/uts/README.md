...
Sources are arranged per-*platform* ('intel'), *implementation* (i86pc), etc,
and by (loosely speaking) function.  `usr/src/uts/common/io/foo/` is likely to
be where the platform independent `foo` driver sources, and their common
Makefile live.
...
only two levels deep, the most-general platform a module can serve, and then
the name of that module.  To carry on our previous examples
`usr/src/uts/intel/foo` would be where the common `foo` driver would be built
for intel platforms, `usr/src/uts/intel/arcmsr` where the arcmsr(4D) would be
built.
...
- `UTSMACH`
  The platform for which this is being built (often the basename of
  this Makefile's parent directory)
...
sources from the current directory for this driver.
...
The directory in which the module configuration file can be found, defaults to
`$(MOD_SRCDIR)`.
