...
pci_common_get_reg_prop(dev_info_t *dip, pci_regspec_t *pci_rp)
{
	...
	for (i = 0; i < number; i++) {
		if ((assigned_addr[i].pci_phys_hi & PCI_CONF_ADDR_MASK) ==
		    phys_hi) {
			uintptr_t addr =
			    (((uintptr_t)assigned_addr[i].pci_phys_mid << 32) |
			    (assigned_addr[i].pci_phys_low));
			uintptr_t size =
			    (((uintptr_t)assigned_addr[i].pci_size_hi << 32) |
			    (assigned_addr[i].pci_size_low));
			 ...
			 * device.
			 */
			if (!pci_prd_valid_busaddr(addr, size)) {
