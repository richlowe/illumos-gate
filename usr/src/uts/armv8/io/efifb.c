 ...
 */
int efifb_force_suspend = 0;

static int efifb_open(dev_t *, int, int, cred_t *);
static int efifb_close(dev_t, int, int, cred_t *);
static int efifb_ioctl(dev_t, int, intptr_t, int, cred_t *, int *);
static int efifb_devmap(dev_t, devmap_cookie_t, offset_t, size_t,
			    size_t *, uint_t);

static struct cb_ops cb_efifb_ops = {
	efifb_open,		/* cb_open */
	efifb_close,		/* cb_close */
	nodev,			/* cb_strategy */
	nodev,			/* cb_print */
	nodev,			/* cb_dump */
	nodev,			/* cb_read */
	nodev,			/* cb_write */
	efifb_ioctl,		/* cb_ioctl */
	efifb_devmap,		/* cb_devmap */
	nodev,			/* cb_mmap */
...
static int efifb_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
		void **result);
static int efifb_attach(dev_info_t *, ddi_attach_cmd_t);
static int efifb_detach(dev_info_t *, ddi_detach_cmd_t);

static struct dev_ops efifb_ops = {
	DEVO_REV,		/* devo_rev */
	0,			/* devo_refcnt */
	efifb_info,		/* devo_getinfo */
	nulldev,		/* devo_identify */
	nulldev,		/* devo_probe */
	efifb_attach,		/* devo_attach */
	efifb_detach,		/* devo_detach */
	nodev,			/* devo_reset */
	&cb_efifb_ops,	/* devo_cb_ops */
...
static void	*efifb_softc_head;

/* Loadable Driver stuff */

static struct modldrv modldrv = {
	&mod_driverops,		/* Type of module.  This one is a driver */
	"UEFI framebuffer driver",	/* Name of the module. */
	&efifb_ops,		/* driver ops */
};
...
_init(void)
{
	...
	if ((e = ddi_soft_state_init(&efifb_softc_head,
		    sizeof (struct efifb_softc), 1)) != 0) {
	    return (e);
	}
	...
	if (e) {
		ddi_soft_state_fini(&efifb_softc_head);
	}
	return (e);
}
...
#define	getsoftc(instance) ((struct efifb_softc *)	\
			ddi_get_soft_state(efifb_softc_head, (instance)))

#define	STREQ(a, b)	(strcmp((a), (b)) == 0)

static int
efifb_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	struct efifb_softc *softc;
	...
	case DDI_RESUME:
		/*
		 * Though efifb doesn't really know how to resume
	...
	if (ddi_soft_state_zalloc(efifb_softc_head, unit) != DDI_SUCCESS) {
		return (DDI_FAILURE);
	}
	softc = getsoftc(unit);
	softc->gfxp_state = gfxp_fb_softc_alloc();
	if (softc->gfxp_state == NULL) {
		(void) ddi_soft_state_free(efifb_softc_head, unit);
		return (DDI_FAILURE);
	}
	...
	(void) efifb_detach(devi, DDI_DETACH);
	return (error);
}

static int
efifb_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	int instance = ddi_get_instance(devi);
	struct efifb_softc *softc = getsoftc(instance);
	...
	case DDI_DETACH:
		...
		(void) ddi_soft_state_free(efifb_softc_head, instance);
		return (DDI_SUCCESS);

	case DDI_SUSPEND:
		/*
		 * This is a generic file, and therefore, cannot
		 ...
		 */
		if (efifb_force_suspend != 0)
			return (gfxp_fb_detach(devi, cmd, softc->gfxp_state));
		else
			return (DDI_FAILURE);

	default:
		cmn_err(CE_WARN, "efifb_detach: unknown cmd 0x%x\n", cmd);
		return (DDI_FAILURE);
	}
}
...
/*ARGSUSED*/
static int
efifb_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg, void **result)
{
	dev_t dev;
	int error;
	int instance;
...
static int
efifb_open(dev_t *devp, int flag, int otyp, cred_t *cred)
{
	struct efifb_softc *softc = getsoftc(DEV2INST(*devp));
...
static int
efifb_close(dev_t devp, int flag, int otyp, cred_t *cred)
{
	struct efifb_softc *softc = getsoftc(DEV2INST(devp));
...
efifb_ioctl(
    ...
    int *rval)
{
	struct efifb_softc *softc = getsoftc(DEV2INST(dev));
	int err;
	...
	case AGPMASTER_MINOR:
		 ...
		 */
		cmn_err(CE_NOTE, "!efifb wants agpmaster");
		return (EBADF);
...
static int
efifb_devmap(dev_t dev, devmap_cookie_t dhp, offset_t off, size_t len,
    size_t *maplen, uint_t model)
{
	...
	if (softc == NULL) {
		cmn_err(CE_WARN, "efifb: Can't find softstate");
		return (-1);
	}
