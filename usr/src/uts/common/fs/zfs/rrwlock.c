...
rrn_find(rrwlock_t *rrl)
{
	...
	if (trackcount_count(&rrl->rr_linked_rcount) == 0)
		return (NULL);
...
rrw_init(rrwlock_t *rrl, boolean_t track_all)
{
	...
	rrl->rr_writer = NULL;
	trackcount_create(&rrl->rr_anon_rcount);
	trackcount_create(&rrl->rr_linked_rcount);
...
rrw_destroy(rrwlock_t *rrl)
{
	...
	trackcount_destroy(&rrl->rr_anon_rcount);
	trackcount_destroy(&rrl->rr_linked_rcount);
}
...
rrw_enter_read_impl(rrwlock_t *rrl, boolean_t prio, void *tag)
{
	...
	ASSERT(trackcount_count(&rrl->rr_anon_rcount) >= 0);

	while (rrl->rr_writer != NULL || (rrl->rr_writer_wanted &&
	    trackcount_is_zero(&rrl->rr_anon_rcount) && !prio &&
	    rrn_find(rrl) == NULL))
	...
	if (rrl->rr_writer_wanted || rrl->rr_track_all) {
		...
		rrn_add(rrl, tag);
		(void) trackcount_add(&rrl->rr_linked_rcount, tag);
	} else {
		(void) trackcount_add(&rrl->rr_anon_rcount, tag);
	}
	ASSERT(rrl->rr_writer == NULL);
	mutex_exit(&rrl->rr_lock);
}
...
rrw_enter_write(rrwlock_t *rrl)
{
	...
	while (trackcount_count(&rrl->rr_anon_rcount) > 0 ||
	    trackcount_count(&rrl->rr_linked_rcount) > 0 ||
	    rrl->rr_writer != NULL) {
...
rrw_exit(rrwlock_t *rrl, void *tag)
{
	mutex_enter(&rrl->rr_lock);
#if !defined(DEBUG) && defined(_KERNEL)
	if (!rrl->rr_writer && rrl->rr_linked_rcount.tc_count == 0) {
		rrl->rr_anon_rcount.tc_count--;
		if (rrl->rr_anon_rcount.tc_count == 0)
...
#endif
	ASSERT(!trackcount_is_zero(&rrl->rr_anon_rcount) ||
	    !trackcount_is_zero(&rrl->rr_linked_rcount) ||
	    rrl->rr_writer != NULL);

	if (rrl->rr_writer == NULL) {
		int64_t count;
		if (rrn_find_and_remove(rrl, tag)) {
			count = trackcount_remove(&rrl->rr_linked_rcount, tag);
		} else {
			ASSERT(!rrl->rr_track_all);
			count = trackcount_remove(&rrl->rr_anon_rcount, tag);
		}
		if (count == 0)
			cv_broadcast(&rrl->rr_cv);
	} else {
...
rrw_held(rrwlock_t *rrl, krw_t rw)
{
	...
	if (rw == RW_WRITER) {
		...
		held = (!trackcount_is_zero(&rrl->rr_anon_rcount) ||
		    rrn_find(rrl) != NULL);
	}
	mutex_exit(&rrl->rr_lock);
