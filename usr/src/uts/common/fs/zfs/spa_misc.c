 ...
 *
 * spa_refcount (per-spa trackcount_t protected by mutex)
 *
...
spa_config_lock_init(spa_t *spa)
{
	for (int i = 0; i < SCL_LOCKS; i++) {
		...
		trackcount_create_untracked(&scl->scl_count);
		scl->scl_writer = NULL;
...
spa_config_lock_destroy(spa_t *spa)
{
	for (int i = 0; i < SCL_LOCKS; i++) {
		...
		trackcount_destroy(&scl->scl_count);
...
spa_config_tryenter(spa_t *spa, int locks, void *tag, krw_t rw)
{
	for (int i = 0; i < SCL_LOCKS; i++) {
		...
		if (rw == RW_READER) {
			...
			if (!trackcount_is_zero(&scl->scl_count)) {
				mutex_exit(&scl->scl_lock);
				spa_config_exit(spa, locks & ((1 << i) - 1),
				    tag);
				return (0);
		...
		(void) trackcount_add(&scl->scl_count, tag);
		mutex_exit(&scl->scl_lock);
	}
	return (1);
}
...
spa_config_enter(spa_t *spa, int locks, void *tag, krw_t rw)
{
	...
	for (int i = 0; i < SCL_LOCKS; i++) {
		...
		if (rw == RW_READER) {
			...
			ASSERT(scl->scl_writer != curthread);
			while (!trackcount_is_zero(&scl->scl_count)) {
...
spa_config_exit(spa_t *spa, int locks, void *tag)
{
	for (int i = SCL_LOCKS - 1; i >= 0; i--) {
		...
		if (!(locks & (1 << i)))
			continue;
		mutex_enter(&scl->scl_lock);
		ASSERT(!trackcount_is_zero(&scl->scl_count));
		if (trackcount_remove(&scl->scl_count, tag) == 0) {
			ASSERT(scl->scl_writer == NULL ||
			    scl->scl_writer == curthread);
...
spa_config_held(spa_t *spa, int locks, krw_t rw)
{
	...
	for (int i = 0; i < SCL_LOCKS; i++) {
		...
		if (!(locks & (1 << i)))
			continue;
		if ((rw == RW_READER && !trackcount_is_zero(&scl->scl_count)) ||
...
spa_add(const char *name, nvlist_t *config, const char *altroot)
{
	...
	trackcount_create(&spa->spa_refcount);
...
spa_remove(spa_t *spa)
{
	...
	ASSERT3U(trackcount_count(&spa->spa_refcount), ==, 0);
	...
	trackcount_destroy(&spa->spa_refcount);
...
{
	ASSERT(trackcount_count(&spa->spa_refcount) >= spa->spa_minref ||
	    MUTEX_HELD(&spa_namespace_lock));
	(void) trackcount_add(&spa->spa_refcount, tag);
}
...
spa_close(spa_t *spa, void *tag)
{
	ASSERT(trackcount_count(&spa->spa_refcount) > spa->spa_minref ||
	    MUTEX_HELD(&spa_namespace_lock));
	(void) trackcount_remove(&spa->spa_refcount, tag);
}
...
spa_refcount_zero(spa_t *spa)
{
	...
	return (trackcount_count(&spa->spa_refcount) == spa->spa_minref);
}
