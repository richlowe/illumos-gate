...
pie_LDFLAGS.cmd= 	$(ZPIE) $(C_PIEFLAGS)
exec_LDFLAGS.cmd=
shared_LDFLAGS.cmd=	$(ZPIC) $(C_PICFLAGS)
pie_CFLAGS=		$(C_PIEFLAGS)
shared_CFLAGS=		$(C_PICFLAGS)
exec_CFLAGS=

LDFLAGS.cmd = \
	$(BDIRECT) $(ENVLDFLAGS1) $(ENVLDFLAGS2) $(ENVLDFLAGS3) \
	$(MAPFILE.NES:%=-M%) $(MAPFILE.PGA:%=-M%) $(MAPFILE.NED:%=-M%) \
	$($(EXECTYPE)_LDFLAGS.cmd)
CFLAGS += $($(EXECTYPE)_CFLAGS)
CCFLAGS += $($(EXECTYPE)_CFLAGS)
CFLAGS64 += $($(EXECTYPE)_CFLAGS)
CCFLAGS64 += $($(EXECTYPE)_CFLAGS)
