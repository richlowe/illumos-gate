...
ld_targ_init_sparc(void)
{
	static const Target _ld_targ = {
...
#if	defined(_ELF64)
			.m_def_interp = MSG_ORIG(MSG_PTH_RTLD_SPARCV9),
#else
		...
		.t_id = {
			...
			.id_got = M_ID_GOT,
			.id_gotdata = M_ID_GOTDATA,
			.id_hash = M_ID_HASH,
			.id_interp = M_ID_INTERP,
			.id_lbss = M_ID_UNKNOWN, /* unused */
		...
		.t_mr = {
			...
			.mr_allocate_got = ld_allocate_got,
			.mr_fillin_gotplt = ld_fillin_gotplt,
		},
		.t_ms = {
			.ms_reg_check = ld_reg_check_sparc,
			.ms_mach_sym_typecheck = ld_mach_sym_typecheck_sparc,
			.ms_is_regsym = ld_is_regsym_sparc,
			.ms_reg_find = ld_reg_find_sparc,
			.ms_reg_enter = ld_reg_enter_sparc,
		}
	};
