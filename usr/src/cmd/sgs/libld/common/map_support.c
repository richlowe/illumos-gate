...
ld_map_sym_enter(Mapfile *mf, ld_map_ver_t *mv, ld_map_sym_t *ms, Ass_desc *ma)
{
	...
	if ((sdp = ld_sym_find(ms->ms_name, hash, &where, ofl)) == NULL) {
		...
		/*
		 * 50 is somewhat randomly chosen by averaging the number of
		 * symbols in illumos mapfiles, and adding some slop.
		 */
		if (aplist_append(&mf->mf_syms, sdp, 50) == NULL)
			return (FALSE);
		...
		if (ma != NULL && ma->ass_enabled != 0) {
			...
			if ((m = libld_calloc(sizeof (Ass_desc), 1)) == NULL)
				return (FALSE);
