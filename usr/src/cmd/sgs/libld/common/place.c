 ...
 *
 * Sections are named .<main>.<thingy> where <main> is the usual section to
 * which it should be mapped, and <thingy> is providing the unique name for
 * the original section (which may itself contain periods).
 *
 * In cases where the usual section would contain periods, <main> will above
 * (such as .data.rel.ro) (XXX: data.rel.ro.local being grouped together...)
 *
    ...
    int ident, const char *alt_os_name)
{
	 ...
	 */
	/* XXX: Is there a better way to condition this? */
	/* XXX: introduce a flag to disable this? */
	if (((ofl->ofl_flags & FLG_OF_RELOBJ) == 0) &&
	    (isp->is_name == oname) &&
	 ...
	 * up from the Mapfile.
	 *
	 * XXX: This might be a good way to implement the .local semantics
	 *
