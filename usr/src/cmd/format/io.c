 ...
 */
intptr_t
input(int type, char *promptstr, int delim, u_ioparam_t *param, void *deflt,
    int cmdflag)
{
...
reprompt:
	...
	if (deflt != NULL) {
		...
		case FIO_MSTR:
			...
			for (i = 0, str = strings; i < *(int *)deflt; i++, str++)
				;
			fmt_print("[%s]", *str);
			break;
		case FIO_OSTR:
		...
		case FIO_SLIST:
			 ...
			 */
			s = find_string(param->io_slist, *(int *)deflt);
			if (s == NULL) {
...
"Please specify units in either b(blocks), c(cylinders), e(end cylinder),\n");
	...
	case FIO_EFI:
		...
		case 't':
			...
			return (intptr_t)((float)nmegs * 1024.0 *
