    ...
    const pthread_mutexattr_t *_RESTRICT_KYWD attr)
{
	 ...
	 * the mutex before pthread_mutex_init() is called.  Sun mutexes, upon
	 * which this these are built and which mutex_init() below represents
	 ...
	 * mutex return EBUSY.
	 *
	 * We respect both these behaviors by here in the POSIX
	 * implementation zeroing the mutex if and only if the mutex magic is
	 * incorrect, and the mutex is robust.
	 */
