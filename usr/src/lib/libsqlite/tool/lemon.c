...
typedef enum {LB_FALSE=0, LB_TRUE} Boolean;
...
struct lemon *lemp;
{
  ...
  for(i=0; i<lemp->nsymbol; i++){
    lemp->symbols[i]->lambda = LB_FALSE;
  }
  ...
  do{
    ...
    for(rp=lemp->rule; rp; rp=rp->next){
      ...
      for(i=0; i<rp->nrhs; i++){
         if( rp->rhs[i]->lambda==LB_FALSE ) break;
      }
      if( i==rp->nrhs ){
        rp->lhs->lambda = LB_TRUE;
        progress = 1;
      }
    }
  }while( progress );
    ...
    for(rp=lemp->rule; rp; rp=rp->next){
      ...
      for(i=0; i<rp->nrhs; i++){
        ...
        if( s2->type==TERMINAL ){
          ...
          if( s1->lambda==LB_FALSE ) break;
	}else{
          progress += SetUnion(s1->firstset,s2->firstset);
          if( s2->lambda==LB_FALSE ) break;
	}
      }
    }
  }while( progress );
...
struct lemon *lemp;
{
  ...
  ** start nonterminal.  */
  ...
  for(rp=lemp->rule; rp; rp=rp->next) rp->canReduce = LB_FALSE;
  for(i=0; i<lemp->nstate; i++){
    struct action *ap;
    for(ap=lemp->sorted[i]->ap; ap; ap=ap->next){
      if( ap->type==REDUCE ) ap->x.rp->canReduce = LB_TRUE;
    }
  }
  for(rp=lemp->rule; rp; rp=rp->next){
