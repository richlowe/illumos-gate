...
static const long double two113 = 10384593717069655257060992658440192.0L,
...
logl_x(long double x, long double *w)
{
	...
	t += s * (v * (A2 + v * (A3 + v * (A4 + v * (A5 + v * (A6 + v *
	    A7))))));
...
powl(long double x, long double y)
{
	...
	if ((ahy | ly) == 0)
		return (one);		/* x**+-0 = 1 */
	else if (hx == 0x3fff0000 && lx == 0 && (__xpg6 & _C99SUSv3_pow) != 0)
		return (one);		/* C99: 1**anything = 1 */
	else if (ahx > 0x7fff0000 || (ahx == 0x7fff0000 && lx != 0) || ahy >
	    0x7fff0000 || (ahy == 0x7fff0000 && ly != 0))
	...
	m = k >> 5;
	{
		...
		long double t1 = 1.666666666666666666666666666660876387437e-1L,
